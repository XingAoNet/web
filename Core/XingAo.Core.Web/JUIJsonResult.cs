...
*  陈成杰 2015年4月26日 ：改用MsgResult用于支持MVC下的返回信息。 
* Last Update Time:
* Update Description:
******************************************************************/
using System.Web;
...
namespace XingAo.Core.Web
{
    ...
    public static class JUIJsonResult
    {
       ...
       /// <param name="navTabId"></param>
        ...
        public static MsgResultModel ShowResult(string msg, string navTabId, StateCode state, string backurl)
        {
            return new MsgResultModel() { message = msg, navTabId = navTabId, statusCode = (int)state, forwardUrl = backurl };
        }
    }
}
