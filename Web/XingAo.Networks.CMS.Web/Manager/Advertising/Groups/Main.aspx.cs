...
namespace XingAo.Networks.CMS.Web.Manager.Advertising.Groups
{
    public partial class Main : Common.BaseListPage
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                var where = QueryBuilder.Create<Model.AdvertisingGroup>()
                ...
                Rep_List.DataSource = uk.LoadWhereLambda<Model.AdvertisingGroup>(where.Expression, p => p.OrderByDescending(c => c.ID), PageNum, PageSize, out r).ToList();
