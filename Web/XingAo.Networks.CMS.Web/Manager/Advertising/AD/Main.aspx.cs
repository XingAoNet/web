...
namespace XingAo.Networks.CMS.Web.Manager.Advertising.AD
{
    public partial class Main : Common.BaseListPage
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                var where = QueryBuilder.Create<Model.Advertising>()
                ...
                Rep_List.DataSource = uk.LoadWhereLambda<Model.Advertising>(where.Expression, p => p.OrderByDescending(c => c.ID), PageNum, PageSize, out r).ToList();
