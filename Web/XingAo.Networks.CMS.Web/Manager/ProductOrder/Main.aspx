<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="Main.aspx.cs" Inherits="XingAo.Networks.CMS.Web.Manager.ProductOrder.Main" %>
<%@ Import Namespace="XingAo.Core" %>
<%@ Import Namespace="XingAo.Networks.CMS.Model" %>
...
<div class="pageHeader">
	...
	<div class="searchBar">
		<table class="searchContent">
			<tr>
				<td>
					订单号：<input type="text" name="keyString" value="<%=keyString %>" />
				</td>
			</tr>
		</table>
		<div class="subBar">
...
<div class="pageContent">
	<div class="panelBar">
		<ul class="toolBar">
			<li><a class="icon" href="<%=Request.GetPath() %>/Edit.aspx" target="navTab"><span>打印所选</span></a></li>
	...
	<table class="table" width="100%" layoutH="138">
		...
		<tbody>
			...
			<tr target="sid" rel="<%#Eval("ID") %>">
				...
				<td><a href="<%=Request.GetPath() %>/OrderInfo/Edit.aspx?BID=<%#Eval("ID") %>" target="navTab" title="查看订单详细"><%#Eval("OrderCode")%></a></td>
				<td>￥<%#Eval("TotalPrice")%></td>
				<td>￥<%#Eval("RebateTotalPrice")%></td>
				<td><a href="WebUsers/Edit.aspx?ID=<%#Eval("BaseWebUsers.ID") %>" target="dialog" width="720" height="380" title="查看会员信息" rel="member_view"><%#Eval("BaseWebUsers.UserName")%></a></td>	
				<td><%#int.Parse(Eval("State").ToString()).ToEnumDescription<OrderStateEnum>() %></td>	
                <td><%#int.Parse(Eval("ManagerState").ToString()).ToEnumDescription<OrderManagerStateEnum>()%></td>			
