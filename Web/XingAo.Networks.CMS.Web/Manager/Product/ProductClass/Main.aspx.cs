...
namespace XingAo.Networks.CMS.Web.Manager.Product.ProductClass
{
    public partial class Main : Common.BaseListPage
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                var where = QueryBuilder.Create<Model.Product_Class>()
                    .Like(c => c.ClassName, keyString);
                ...
                Rep_List.DataSource = uk.LoadWhereLambda<Model.Product_Class>(where.Expression, p => p.OrderBy(c=>c.ClassCode).ThenBy(c => c.ID), PageNum, PageSize, out r).ToList();
