...
namespace XingAo.Networks.CMS.Web.Manager.Product.ProductAttribute.AttributeSet
{
    public partial class Main : Common.BaseListPage
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                var where = QueryBuilder.Create<Model.Product_Attribute>()
                    .Like(c => c.AttrName, keyString);
                ...
                Rep_List.DataSource = uk.LoadWhereLambda<Model.Product_Attribute>(where.Expression, p => p.OrderByDescending(c => c.GroupID).ThenByDescending(c=>c.OrderNum).ThenByDescending(c=>c.ID), PageNum, PageSize, out r).ToList();
