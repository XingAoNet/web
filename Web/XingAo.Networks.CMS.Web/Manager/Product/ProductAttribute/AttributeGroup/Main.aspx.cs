...
namespace XingAo.Networks.CMS.Web.Manager.Product.ProductAttribute.AttributeGroup
{
    public partial class Main : Common.BaseListPage
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                var where = QueryBuilder.Create<Model.Product_AttributeGroup>()
                ...
                Rep_List.DataSource = uk.LoadWhereLambda<Model.Product_AttributeGroup>(where.Expression, p => p.OrderByDescending(c => c.ID), PageNum, PageSize, out r).ToList();
