...
namespace XingAo.Networks.CMS.Web.Manager.FriendLink.Group
{
    public partial class Main : Common.BaseListPage
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                var where = QueryBuilder.Create<Model.FriendLinkGroup>()
                ...
                Rep_List.DataSource = uk.LoadWhereLambda<Model.FriendLinkGroup>(where.Expression,p => p.OrderByDescending(c=>c.ID), PageNum, PageSize, out r).ToList();
