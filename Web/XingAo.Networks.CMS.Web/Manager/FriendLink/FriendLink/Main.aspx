<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="Main.aspx.cs" Inherits="XingAo.Networks.CMS.Web.Manager.FriendLink.FriendLink.Main" %>
...
<div class="pageContent">
	<div class="panelBar">
		<ul class="toolBar"> 
			<li><a class="add" href="<%=Request.GetPath() %>/Edit.aspx?MenuID=<%=Request.QueryString["MenuID"]%>" target="dialog" width="1000" height="500" title="添加友情链接" rel="linkfriend_add"><span>添加</span></a></li>
			<li><a title="确实要删除这些记录吗?" target="selectedTodo" rel="ids" href="<%=Request.GetPath() %>/SaveDel.ashx" class="delete"><span>删除所选</span></a></li>
			<li><a class="edit" href="<%=Request.GetPath() %>/Edit.aspx?MenuID=<%=Request.QueryString["MenuID"]%>&id={sid}" target="dialog" width="1000" height="500" title="修改友情链接" rel="linkfriend_edit" warn="请选择一条数据"><span>修改</span></a></li>
	...
	<table class="table" width="100%" layoutH="138">
		...
		<tbody>
            ...
            <ItemTemplate>            
			<tr target="sid" rel="<%#Eval("ID") %>" height="80">
				<td><input name="ids" value="<%#Eval("ID") %>" type="checkbox"></td>
				<td><%#Eval("Title")%></td>
                <td><%#Eval("Groups.GroupName")%></td>
<td><a href="ShowPic.aspx?pic=<%#Eval("Pic")%>" target="dialog" width="603" height="413" title="查看原图"><img src="<%#Eval("Pic")%>" style="max-height:80px;max-width:300px" alt="<%#Eval("Pic")%>" /></a></td>
