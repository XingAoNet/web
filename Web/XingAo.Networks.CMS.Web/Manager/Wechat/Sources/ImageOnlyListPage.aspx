<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="ImageOnlyListPage.aspx.cs" Inherits="XingAo.Networks.CMS.Web.Manager.Wechat.Sources.ImageOnlyListPage" %>
	    ...
	    <thead>
            ...
            <asp:repeater runat="server" ID="Rep_List">
                <ItemTemplate> 
                    <tr>
                        <td>
					    <a class="btnSelect" href="javascript:$.bringBack({txtKeys:'<%#Eval("MFGuid")%>', txtKeys_text:'<%#Eval("Name")%>','txtSource':'<%=Request.QueryString["Source"]%>','txtAutoId':'<%#Eval("ID")%>'})" title="查找带回">选择</a>
				        </td>
                        <td><%#Eval("Name")%></td>
                        <td><%#Eval("Describe")%></td>
                    </tr>  
