...
namespace XingAo.Networks.CMS.Web.Manager.LableDesign.SysLable
{
    public partial class Main : Common.BaseListPage
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                var where = QueryBuilder.Create<Model.SysLabels>()
                   .Like(c => c.LabelName, keyString);
                ...
                Rep_List.DataSource = uk.LoadWhereLambda<Model.SysLabels>(where.Expression, p => p.OrderByDescending(c => c.ID), PageNum, PageSize, out r).ToList();
