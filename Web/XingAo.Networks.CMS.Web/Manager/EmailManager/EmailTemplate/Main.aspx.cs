...
namespace XingAo.Networks.CMS.Web.Manager.EmailManager.EmailTemplate
{
    public partial class Main : Common.BaseListPage
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                var where = QueryBuilder.Create<Model.EmailTemplateModel>()
                ...
                Rep_List.DataSource = uk.LoadWhereLambda<Model.EmailTemplateModel>(where.Expression, p => p.OrderByDescending(c => c.ID), PageNum, PageSize, out r).ToList();
