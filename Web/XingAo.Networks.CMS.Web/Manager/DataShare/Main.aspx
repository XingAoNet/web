<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="Main.aspx.cs" Inherits="XingAo.Networks.CMS.Web.Manager.DataShare.Main" %>
...
<div class="pageHeader">
	...
	<div class="searchBar">
		<table class="searchContent">
			<tr>
				<td>
					方法名：<input type="text" name="keyString" value="<%=keyString %>" />
				</td>
			</tr>
		</table>
		<div class="subBar">
...
<div class="pageContent">
	<div class="panelBar">
		<ul class="toolBar"> 
            ...
            <li><a class="add" href="<%=Request.GetPath() %>/Edit.aspx?MenuID=<%=Request.QueryString["MenuID"]%>" target="dialog" width="1000" height="500" title="添加接口调用" rel="datashare_add"><span>添加</span></a></li>
			<li><a title="确实要删除这些记录吗?" target="selectedTodo" rel="ids" href="<%=Request.GetPath() %>/SaveDel.ashx" class="delete"><span>删除所选</span></a></li>
			<li><a class="edit" href="<%=Request.GetPath() %>/Edit.aspx?MenuID=<%=Request.QueryString["MenuID"]%>&id={sid}"  warn="请选择一条数据" target="dialog" width="1000" height="500" title="修改接口调用" rel="datashare_edit"><span>修改</span></a></li>
	...
	<table class="table" width="100%" layoutH="138">
		...
		<tbody>
			...
			<tr target="sid" rel="<%#Eval("ID") %>">
				...
				<td><a href="<%=Request.GetPath() %>/ParameterConf/Main.aspx?DataShareID=<%#Eval("ID") %>" target="navTab" rel="ParameterConfnavTab" title="<%#Eval("MethodName")%> 附加条件信息"><%#Eval("MethodName")%></a></td>
				<td><%#GetMethodType(Eval("MethodType").ToString())%></td>
				<td><%#Eval("Tables")%></td>	
                <td><%#Eval("Fields")%></td>
                <td><%#Eval("WhereStr")%></td>
                <td><%#Eval("OrderBy")%></td>
