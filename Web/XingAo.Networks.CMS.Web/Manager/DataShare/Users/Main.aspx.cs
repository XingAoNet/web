...
namespace XingAo.Networks.CMS.Web.Manager.DataShare.Users
{
    public partial class Main : Common.BaseListPage
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                var where = QueryBuilder.Create<Model.DataShare_Users>()
                ...
                Rep_List.DataSource = uk.LoadWhereLambda<Model.DataShare_Users>(where.Expression, p => p.OrderByDescending(c => c.ID), PageNum, PageSize, out r).ToList();
