...
namespace XingAo.Networks.CMS.Web.Manager.SysConfigs.ManagerUser.Groups
{
    public partial class Main : Common.BaseListPage
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                var where = QueryBuilder.Create<Model.ManagerUserGroup>()
                ...
                Rep_List.DataSource = uk.LoadWhereLambda<Model.ManagerUserGroup>(where.Expression, p => p.OrderByDescending(c => c.ID), PageNum, PageSize, out r).ToList();
