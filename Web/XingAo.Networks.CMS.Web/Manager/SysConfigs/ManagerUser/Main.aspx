<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="Main.aspx.cs" Inherits="XingAo.Networks.CMS.Web.Manager.SysConfigs.ManagerUser.Main" %>
...
<div class="pageContent">
	<div class="panelBar">
		<ul class="toolBar"> 
			<li><a class="add" href="<%=Request.GetPath() %>/Edit.aspx?MenuID=<%=Request.QueryString["MenuID"]%>" target="dialog" width="1000" height="500" title="添加管理员信息" rel="manager_user_add"><span>添加</span></a></li>
			<li><a title="确实要删除这些记录吗?" target="selectedTodo" rel="ids" href="<%=Request.GetPath() %>/SaveDel.ashx" class="delete"><span>删除所选</span></a></li>
			<li><a class="edit" href="<%=Request.GetPath() %>/Edit.aspx?MenuID=<%=Request.QueryString["MenuID"]%>&id={sid}" target="dialog" width="1000" height="500" title="修改管理员信息" rel="manager_user_edit" warn="请选择一条数据"><span>修改</span></a></li>
	...
	<table class="table" width="100%" layoutH="138">
		...
		<tbody>
			...
			<tr target="sid" rel="<%#Eval("ID") %>">
				...
				<td><%#Eval("UserName")%></a></td>
				<td><%#Eval("realname")%></td>
				<td><%#Eval("LastLogin","{0:yyyy-MM-dd HH:mm}")%></td>
                <td><%#Eval("CanUse").ObjToInt()==0?"禁用":"可用"%></td>				
			</tr>
            </ItemTemplate>
            </asp:repeater>
		</tbody>
