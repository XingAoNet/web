...
namespace XingAo.Networks.CMS.Web.Manager.SysConfigs.ManagerUser
{
    public partial class Main : Common.BaseListPage
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                var where = QueryBuilder.Create<Model.ManagerUser>()
                ...
                Rep_List.DataSource = uk.LoadWhereLambda<Model.ManagerUser>(where.Expression, p => p.OrderByDescending(c => c.ID), PageNum, PageSize, out r).ToList();
