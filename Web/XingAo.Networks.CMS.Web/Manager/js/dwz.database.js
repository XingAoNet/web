...
(function ($) {
    ...
    $.extend({
        bringBackSuggest: function (args) {
            ...
            $box.find(":input").each(function () {
                ...
                for (var key in args) {
                    ...
                    //                        break;
                    //                    }
                    //2013年11月2日 16:38:36 卢小阳 注释以上代码，将查找并返回改用控件的name值与这里的返回json的key对应即返回
                    if (inputName == key) {
                        $input.val(args[key]);
                        break;
                    }
        ...
        //2013年9月21日 17:23:42 卢小阳，扩展jui的查找带返回  的返回函数以支持json或数组方式返回
        CallBackSetFormValue: function (args) {
            if (args instanceof Array)//如果参数是数组，则直接循环赋值
            {//alert("array");
                $("input[lookFor='" + _lookup.currentGroup + "']").each(function (index, element) {
                    var $this = $(element);
                    $this.val(args[index]);
                });
            }
            else//如果是json，则以key对应的方式赋值
            {//alert("json");
                for (var key in args) {
                    $("input[lookFor='" + _lookup.currentGroup + "'][name='" + key + "']").val(args[key]);
                }
            }
            $.pdialog.closeCurrent();
        }
        ...
        itemDetail: function () {
            return this.each(function () {
                ...
                if (addButTxt) {
                    ...
                    var $div = $("<div class='ItemAddBtn'></div>").insertBefore($table);

                    var $addBut = $('<div class="button"><div class="buttonContent"><button type="button">' + addButTxt + '</button></div></div>').appendTo($div).find("button");
                    var $rowNum = $('<input type="text" name="dwz_rowNum" class="textInput" style="margin:2px;" value="1" size="2"/>').appendTo($div);
                    var trTm = "";
            ...
            function tdHtml(field) {
                ...
                switch (field.type) {
                    ...
                    case 'lookup':
                        ...
                        //html = '<input type="hidden" name="' + field.lookupGroup + '.' + field.lookupPk + suffix + '"/><input type="hidden" name="DbID" value="0"/>'
                        html = '<input type="hidden" name="' + field.lookupPk + suffix + '"/><input type="hidden" name="DbID" value="0"/>'
