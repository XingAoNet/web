                         ...
                         if (checkFn && $.isFunction(checkFn))
                         {
                             ...
                             function ()
                             {
                                 ...
                                 ckbox.click(function ()
                                 {
                                     ...
                                     var items = [];
//                                     if (checked)//因项目需要：取消选择时也能返回items（即取消选中的项）特注释此处条件判断
//                                     {
                                         ...
                                         if (boxes.size() > 1)
                                         {
                                             $(boxes).each(function ()
                                             {
                                                 items[items.length] = {check:checked, name: $(this).attr("name"), value: $(this).val(), text: $(this).attr("text") };
                                             });
                                         }
                                         else
                                         {
                                             items = { check: checked, name: boxes.attr("name"), value: boxes.val(), text: boxes.attr("text") };
                                         }
//                                     }
