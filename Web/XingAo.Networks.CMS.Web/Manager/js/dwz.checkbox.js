...
(function($){
	...
	$.checkbox = {
		...
		select: function(_name, _type, _parent){
			...
			$checkboxLi = $parent.find(":checkbox[name='"+_name+"'][disabled!='disabled']");//2013年9月28日 08:53:45 卢小阳 将此处$checkboxLi = $parent.find(":checkbox[name='"+_name+"']");改成$checkboxLi = $parent.find(":checkbox[name='"+_name+"'][disabled!='disabled']");   目的在于：在列表页上点击全选时，不选中已禁用的多选框
			switch(_type){
				case "invert":
