<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="Main.aspx.cs" Inherits="XingAo.Networks.CMS.Web.Manager.WebUsers.Main" %>
...
<div class="pageContent">
	<div class="panelBar">
		<ul class="toolBar">
			<li><a class="add" href="<%=Request.GetPath() %>/Edit.aspx" target="dialog" width="760" height="440" title="添加会员信息" rel="usermanager_add"><span>添加</span></a></li>
			<li><a title="确实要删除这些记录吗?" target="selectedTodo" rel="ids" href="<%=Request.GetPath() %>/SaveDel.ashx" class="delete"><span>删除所选</span></a></li>
			<li><a class="edit" href="<%=Request.GetPath() %>/Edit.aspx?id={sid}" target="dialog" width="760" height="440" title="修改会员信息" rel="usermanager_edit" warn="请选择一条数据"><span>修改</span></a></li>
	...
	<table class="table" width="100%" layoutH="138">
		...
		<tbody>
			...
			<tr target="sid" rel="<%#Eval("ID") %>">
				...
				<td><a href="<%=Request.GetPath() %>/Edit.aspx?ID=<%#Eval("ID") %>" target="navTab"  title="会员详细"><%#Eval("UserName")%></a></td>
				<td><%#Eval("RealName")%></td>
				<td><%#Eval("Points")%></td>
                <td><%#Eval("VipLevel")%></td>	
                <td><%#Eval("Mobile")%></td>
                <td><a href="mailto:<%#Eval("Email")%>"><%#Eval("Email")%></a></td>
                <td align="center"><%#Eval("CanUse").ToString() == "0" ? "禁用" : "可用"%></td>
                <td align="center"><%#Eval("Audit").ToString()=="0"?"未审核":"已审核"%></td>			
