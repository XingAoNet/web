...
namespace XingAo.Networks.CMS.Web.Manager.Members.Groups//----------修改命名空间
{
    /// <summary>
    /// SaveDel 的摘要说明
    /// </summary>
    ...
    {
        readonly string navTabId = "Nav_ManagerGroup";//----------修改标签ID
        public void ProcessRequest(HttpContext context)
        {
            ...
            if (!string.IsNullOrEmpty(action))//添加或修改
            {
                ...
                if (uk.FindBySpecification<Model.MemberGroups>(p => p.Id != id && p.GroupName == groupName).Count() > 0)
                {
                    callbackaction = "";
                    msg = "组名已存在！";
                    IsV = false;
                ...
                Model.MemberGroups mode;
                if (id > 0)
                    mode = uk.FindSigne<Model.MemberGroups>(p => p.Id == id);
                else
                {
                    mode = new Model.MemberGroups();
                    mode.GroupId = System.Guid.NewGuid().ToString("N");
                }
            ...
            else//删除
            {
                ...
                foreach (Model.MemberGroups g in uk.FindByFunc<Model.MemberGroups>(c => IDs.Contains(c.Id.ToString())).ToList())
                {
                    uk.Remove(g, false);
                }
